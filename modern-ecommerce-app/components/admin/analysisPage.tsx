...
function Metric({ label, value, icon: Icon, change, muted = false }: { label: string; value: string; icon: typeof BarChart3; change?: string; muted?: boolean }) {
  return (
    <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-4">
      <div className="flex items-center justify-between gap-2"><p className="text-xs font-medium text-slate-500">{label}</p><Icon size={16} className="text-emerald-600" /></div>
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><p className="text-sm font-medium text-emerald-600">Business intelligence</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">Analysis dashboard</h2><p className="mt-1 text-sm text-slate-500">A quick view of your store performance and opportunities.</p></div>
        <select className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 outline-none"><option>Last 30 days</option><option>Last 7 days</option><option>Last 12 months</option></select>
      </div>

      <Card><SectionTitle icon={BarChart3} title="Overview / Analytics Dashboard" description="Overall business performance at a glance." /><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Total revenue" value={currency(totalSales)} icon={CircleDollarSign} change="+12.4% vs previous period" /><Metric label="Total sales" value={number(totalSold)} icon={ShoppingBag} change="+8.2% vs previous period" /><Metric label="Total orders" value={number(totalOrders)} icon={PackageCheck} /><Metric label="Total customers" value={number(customers)} icon={Users} /><Metric label="Average order value" value={currency(averageOrder)} icon={WalletCards} /><Metric label="Products sold" value={number(totalSold)} icon={Boxes} /><Metric label="Pending orders" value={number(pending)} icon={Clock3} /><Metric label="Conversion rate" value="—" icon={Target} muted /></div><div className="mt-4 grid gap-3 md:grid-cols-4"><div className="rounded-xl bg-emerald-600 p-4 text-white"><p className="text-xs text-emerald-100">Revenue growth</p><p className="mt-2 text-2xl font-bold">+12.4%</p></div><div className="rounded-xl bg-slate-900 p-4 text-white"><p className="text-xs text-slate-300">Sales growth</p><p className="mt-2 text-2xl font-bold">+8.2%</p></div><div className="rounded-xl border border-slate-100 p-4"><p className="text-xs text-slate-500">Net profit</p><p className="mt-2 text-xl font-bold text-slate-400">—</p><Placeholder label="Cost data" /></div><div className="rounded-xl border border-slate-100 p-4"><p className="text-xs text-slate-500">Refund / return rate</p><p className="mt-2 text-xl font-bold text-slate-400">—</p><Placeholder label="Return data" /></div></div></Card>
